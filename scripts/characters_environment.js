...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona el boton Jugar para empezar ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
  ...
  if(gameConfig.status==='gameover'){
    ...
    textSize(15);
    text("Presiona la tecla espacio para reiniciar", gameConfig.screenX/2, gameConfig.screenY/2+135);
    textSize(40);
...
function changeGameStatud(character){
  if( nariz_x != 0 && jugando && gameConfig.status==="start") {
...
function manualControl(character){
  ...
  if(character.live){
    if(keyDown(control.left) || nariz_x > 220){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }

    if(keyDown(control.right) || nariz_x < 110){
      character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }
...
function jumping(character){
	if( (keyWentDown(control.up)&&character.live) || (touchIsDown&&character.live) || nariz_y < 150 ){
...
function scores(character){
  ...
  text("puntos: "+round(gameConfig.scores),20,40);
